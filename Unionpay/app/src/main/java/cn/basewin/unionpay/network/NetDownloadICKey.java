 ...
 * 描述：IC卡公钥下载
 */
@AnnotationNet(action = ActionConstant.ACTION_DOWNLOAD_ICKEY)
public class NetDownloadICKey extends OrdinaryMessage {
    @Override
    ...
    public boolean afterRequest(Iso8583Manager iso, NetResponseListener lis) {
        try {
            ...
            switch (code) {
                case 0:
                    if (lis != null) {
                        lis.onFailure(ErrorConstant.ERR_PARA, "获取密钥详情失败");
                    }
                    return false;
                case 1:
                    String keyValue = result.substring(2);
                    mPbocService.writeCapk(0, keyValue);
                    break;
            }
        } catch (Exception e) {
            e.printStackTrace();
            if (lis != null) {
                lis.onFailure(ErrorConstant.ERR_PARA, "存储密钥失败");
            }
            return false;
        }
        return super.afterRequest(iso, lis);
    }
}

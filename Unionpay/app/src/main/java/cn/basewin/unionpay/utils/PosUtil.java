...
public class PosUtil {
    private static final String TAG = PosUtil.class.getName();
    ...
    public static String changeAmout(String amtStr) {
        ...
        // 最长12位
        if (str.length() > AppConfig.MAXAMTNUM)
            cuttedStr = cuttedStr.substring(0, AppConfig.MAXAMTNUM);
        Log.d(TAG, "---account2 ---" + cuttedStr);
    ...
    public static String centToYuan(String msg) {
        if (!msg.matches(CURRENCY_FEN_REGEX)) {
            TLog.showToast("金额格式有误");
            return "";
        }
        return BigDecimal.valueOf(Long.valueOf(msg)).divide(new BigDecimal(100)).toString();
    }
...
//            case 5:
    ...
    public static boolean isICBy22(String _22) {
        ...
        try {
            ...
            TLog.e(TAG, "22域转成刷卡类型码时异常！");
        }
        if (s.equals("05") || s.equals("07")) {
            return true;
        }
        return false;
    }
    ...
    public static int parseF55(byte[] F55_Field) {
        ...
        if (TLog.isDebug) {
            Log.e(TAG, "parseF55 ..");
            Log.e(TAG, "F55 len = " + F55_Field.length);
        }
        ...
        for (i = 0; i < F55_Field.length; ) {
            ...
            if (tagInfo != null && tagInfo.length == 3 && F55_Field.length >= (tagInfo[1] + tagInfo[2])) {
                ...
                if (TLog.isDebug)
                    Log.e("PARSE55", "TLVParse [" + StringUtil.toHexString(shortToByte2(tagInfo[0]), false) + "]" + "[" + tagInfo[2] + "][" + StringUtil.toHexString(buffer) + "]");
        ...
        for (i = 0; i < F55_Field.length; ) {
            ...
            if (tagInfo != null && tagInfo.length == 3 && F55_Field.length >= (tagInfo[1] + tagInfo[2])) {
                ...
                for (int k = 0; k < tag.length; k++) {
                    if (StringUtil.toHexString(shortToByte2(tagInfo[0]), false).equals(tag[k])) {
                        if (istag) {
                            sb.append(StringUtil.toHexString(shortToByte2(tagInfo[0]), false)).append(tagInfo[2]).append(StringUtil.toHexString(buffer).toString());
                        } else {
